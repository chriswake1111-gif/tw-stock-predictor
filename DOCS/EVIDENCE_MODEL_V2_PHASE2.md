...
The approval policy is rule-specific: `VAL-02` approves Forward EPS and `VAL-04` approves company-specific PE selection. `VAL-01` is the deterministic multiplication formula. `VAL-03` appears only when the PE record explicitly sets `evidence_basis_rule_id: VAL-03` and uses the documented 20, 21, or 25 multiple; it never borrows the `VAL-04` approval ID. General Forward PE matrices therefore use `VAL-01`, `VAL-02`, and `VAL-04` by default. Rule Trace contains only approval IDs whose ledger event has that exact Rule ID.
...
General runtime installation uses:

```text
python -m pip install -c constraints.txt -r requirements.txt
```

CI and local test environments install `requirements-dev.txt`, which applies the same `constraints.txt`. The compatibility set pins FastAPI, Starlette, Pydantic, httpx2, pytest, and pytest-cov; CI does not perform an additional unconstrained test-tool installation.
