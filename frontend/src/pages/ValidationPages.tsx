...
function formatHistoricalRate(value: string | null): string {
  if (value === null) return "—";
  const parsed = Number(value);
  return Number.isFinite(parsed)
    ? new Intl.NumberFormat("zh-TW", { style: "percent", maximumFractionDigits: 1 }).format(parsed)
    : value;
}
...
export function ValidationRunPage() {
  ...
  return <div className="page"><Link className="back-link" to="/validation">← 返回歷史觀察</Link><header className="workspace-heading"><div><span className="eyebrow">Evaluation run</span><h1>{run.evaluator_version}</h1><p className="muted">{run.evaluation_run_id}</p></div></header><section className="origin-disclosure"><span>各 evaluation origin 均獨立分組呈現，不合併統計。</span></section><div className="performance-grid">{summary.groups.map((group, index) => <article key={`${group.evaluation_origin}-${group.method_family}-${group.horizon_sessions}-${index}`}><header><OriginBadge origin={group.evaluation_origin} /><span>{group.horizon_sessions} sessions</span></header><h2>{group.method_family}</h2><div className="sample-size"><span>有效樣本 n={group.n}</span><NumeratorDenominator numerator={group.numerator} denominator={group.denominator} /></div><dl className="metric-dl"><div><dt>歷史條件下目標區間觸及觀察</dt><dd>{formatHistoricalRate(group.historical_target_reach_rate)}</dd></div><div><dt>Median return</dt><dd>{group.median_forward_return ?? "—"}</dd></div><div><dt>Median excess</dt><dd>{group.median_excess_return ?? "—"}</dd></div></dl><p className="muted">描述性歷史觀察，不是未來預測機率。</p></article>)}</div><section className="disclosure-panel"><BarChart3 aria-hidden="true" /><div><strong>限制</strong>{summary.disclosures.map((item) => <p key={item}>{item}</p>)}</div></section></div>;
}
