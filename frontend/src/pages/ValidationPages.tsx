...
export function ValidationRunPage() {
  ...
  return <div className="page"><Link className="back-link" to="/validation">← 返回歷史觀察</Link><header className="workspace-heading"><div><span className="eyebrow">Evaluation run</span><h1>{run.evaluator_version}</h1><p className="muted">{run.evaluation_run_id}</p></div></header><section className="origin-disclosure"><OriginBadge origin="prospective_snapshot" /><span>前瞻保存與歷史重建在每個群組中分開揭露。</span></section><div className="performance-grid">{summary.groups.map((group, index) => <article key={`${group.evaluation_origin}-${group.method_family}-${group.horizon_sessions}-${index}`}><header><OriginBadge origin={group.evaluation_origin} /><span>{group.horizon_sessions} sessions</span></header><h2>{group.method_family}</h2><div className="sample-size"><span>有效樣本</span><NumeratorDenominator numerator={group.numerator} denominator={group.denominator} /></div><dl className="metric-dl"><div><dt>Target reach</dt><dd>{group.historical_target_reach_rate ?? "—"}</dd></div><div><dt>Median return</dt><dd>{group.median_forward_return ?? "—"}</dd></div><div><dt>Median excess</dt><dd>{group.median_excess_return ?? "—"}</dd></div></dl></article>)}</div><section className="disclosure-panel"><BarChart3 aria-hidden="true" /><div><strong>限制</strong>{summary.disclosures.map((item) => <p key={item}>{item}</p>)}</div></section></div>;
}
