"""Immutable TGT-01 synthesis profile revisions and approval events."""
...
class SynthesisProfileRepository:
    ...
    def _bind(
        ...
        conn.execute(
            "INSERT INTO synthesis_idempotency_keys VALUES (?,?,?,?,?)",
    ...
    def _existing(
        ...
        *,
        ...
        binding = conn.execute(
            "SELECT * FROM synthesis_idempotency_keys WHERE idempotency_key = ?",
            (key,),
        ).fetchone()
        if binding:
            if (
    ...
    def _public_profile(row: dict[str, Any]) -> dict[str, Any]:
        result = dict(row)
        result["allowed_method_families"] = json.loads(
            result.pop("allowed_method_families_json")
        )
        result["evidence_strength_policy"] = json.loads(
            result.pop("evidence_strength_policy_json")
        )
        return result
    ...
    def add_revision(
        ...
        *,
        ...
        record_id = f"synthesis_profile_{fingerprint[:24]}"
        ...
        with self._connect() as conn:
            existing = self._existing(
                ...
                table="synthesis_profile_revisions",
                id_column="id",
            )
            if existing:
                return self._public_profile(existing)
            ...
            if payload["revision_number"] == 1:
                ...
                if previous is None or previous["id"] != payload["revision_of"]:
                    raise ValueError("revision_of must reference the latest synthesis profile")
            ...
            conn.execute(
                """
                INSERT INTO synthesis_profile_revisions (
                    id,idempotency_key,payload_fingerprint,logical_profile_id,
                    revision_number,revision_of,scope,scope_value,
                    allowed_method_families_json,role_compatibility_policy,
                    point_to_range_policy,overlap_tolerance,boundary_policy,
                    cluster_policy,dependency_policy_version,
                    evidence_strength_policy_json,calculation_quantum,
                    display_quantum,rounding_mode,available_at,ingested_at,
                    created_by,rationale,status
                ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                """,
                (
                    record_id,
                    ...
                    payload["scope"],
                    payload["scope_value"],
                    json.dumps(payload["allowed_method_families"], separators=(",", ":")),
                    payload["role_compatibility_policy"],
                    payload["point_to_range_policy"],
                    payload["overlap_tolerance"],
                    payload["boundary_policy"],
                    payload["cluster_policy"],
                    payload["dependency_policy_version"],
                    json.dumps(payload["evidence_strength_policy"], separators=(",", ":")),
                    payload["calculation_quantum"],
                    payload["display_quantum"],
            ...
            self._bind(conn, idempotency_key, fingerprint, "profile_revision", record_id)
            result = dict(
                conn.execute(
            ...
            result["created"] = True
            return self._public_profile(result)
    ...
    def add_approval(
        ...
        *,
        ...
        fingerprint = sha256_json(payload)
        event_id = f"synthesis_approval_event_{fingerprint[:20]}"
        ingested = normalize_utc_timestamp(
            ingested_at or utc_now_timestamp(), "ingested_at"
        )
        ...
        with self._connect() as conn:
            existing = self._existing(
                ...
                table="synthesis_profile_approvals",
            ...
            if profile is None:
                raise ValueError("synthesis profile revision does not exist")
            ...
            if (
                ...
                raise ValueError("a revoked synthesis profile cannot be approved")
            previous = conn.execute(
                """
                SELECT approved_at FROM synthesis_profile_approvals
                WHERE profile_revision_id = ? AND rule_id = 'TGT-01'
            ...
            conn.execute(
                """
                INSERT INTO synthesis_profile_approvals (
            ...
            self._bind(conn, idempotency_key, fingerprint, "profile_approval", event_id)
            result = dict(
                conn.execute(
                    "SELECT * FROM synthesis_profile_approvals WHERE approval_event_id = ?",
                    (event_id,),
                ).fetchone()
            )
            result["created"] = True
